...
class MetroAgi:
    ...
    def en_az_aktarma_bul(self, baslangic_id: str, hedef_id: str) -> Optional[List[Istasyon]]:
        ...
        1. Başlangıç ve hedef istasyonların varlığını kontrol edin /yaptım
        ...
        # BFS için kuyruk oluşturma
        kuyruk = deque([(baslangic, [baslangic])])     

        while kuyruk:
            guncelIstasyon, guncelIstasyonaOlanYol = kuyruk.popleft()

            # İstenilen istasyonsa yolu döndür
            if guncelIstasyon == hedef:
                return guncelIstasyonaOlanYol

            # Tek tek komşuları geziyoruz
            for neighbor, _ in guncelIstasyon.komsular:
                if neighbor not in ziyaret_edildi:
                    ziyaret_edildi.add(neighbor)
                    kuyruk.append((neighbor, guncelIstasyonaOlanYol + [neighbor]))
        return None                
    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        while pq:
            guncelZaman, _, guncel, yol  = heapq.heappop(pq)

            # İstenilen istasyona ulaşıldıysa yolu ve süreyi döndürür
            if guncel == hedef:
                return yol, guncelZaman

            # Komşu istasyonları ziyaret edip süreyi hesaplıyoruz
            for komsu, zaman in guncel.komsular:
                if komsu not in ziyaret_edildi:
                    ziyaret_edildi.add(komsu)
                    yeni_zaman = guncelZaman + zaman
                    # A* algoritması için f = g + h kısmı
                    heapq.heappush(pq, (yeni_zaman, id(komsu), komsu, yol + [komsu]))
